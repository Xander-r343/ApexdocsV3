...
<Tabs items={['Java', 'Kotlin']}>
  ...
  <Tabs.Tab>
    ```kotlin
    // Example values (yours will vary)
    val DRIVE_FEED_FORWARD = 0.055
    val STRAFE_FEED_FORWARD = 0.072
    ```
  </Tabs.Tab>
</Tabs>

<Callout type="warning">
  **Warning:** Setting the feedforward **too high** will cause your robot to creep forward unintentionally during autonomous. The goal is the highest value where the robot still does not move.
</Callout>

<Callout type="info">
  **Tip:** Strafe feedforward is almost always higher than drive feedforward since strafing produces more friction on mecanum wheels.
</Callout>
